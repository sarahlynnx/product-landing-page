    ...
    <Navbar expand="false" collapseOnSelect className="mx-3" fixed="top">
      ...
      >
        {!collapsed && (
          <>
            <div>
              <h1 className="nav-header" style={titleStyle}>
                MENU
              </h1>
            </div>
            <div className="main-nav">
              <ScrollSpyNav
                scrollTargetIds={["about", "faqs", "contact"]}
                offset={-120}
              >
                <Nav>
                  <Nav.Link href="#about" onClick={handleMenuOverlay}>
                    ABOUT
                  </Nav.Link>
                  <Nav.Link href="#faqs" onClick={handleMenuOverlay}>
                    FAQS
                  </Nav.Link>
                  <Nav.Link href="#contact" onClick={handleMenuOverlay}>
                    CONTACT
                  </Nav.Link>
                </Nav>
              </ScrollSpyNav>
            ...
            <div className="overlay-product-nav">
              <Nav className="product-nav justify-content-between">
                ...
                >
                  {Object.entries(flavorImages["DARK CHOCOLATE"]).map(
                    ([flavor, data]) => (
                      <NavDropdown.Item
                        key={flavor}
                        onClick={() => {
                ...
                <NavDropdown title="MINI CRISPS" id="mini-crisps-dropdown">
                  {Object.entries(flavorImages["MINI CRISPS"]).map(
                    ([flavor, data]) => (
                      <NavDropdown.Item
                        key={flavor}
                        onClick={() => {
                ...
                >
                  {Object.entries(flavorImages["MILK CHOCOLATE"]).map(
                    ([flavor, data]) => (
                      <NavDropdown.Item
                        key={flavor}
                        onClick={() => {
                ...
                <NavDropdown title="SAMPLER PACKS" id="sampler-packs-dropdown">
                  {Object.entries(flavorImages["SAMPLER PACKS"]).map(
                    ([flavor, data]) => (
                      <NavDropdown.Item
                        key={flavor}
                        onClick={() => {
